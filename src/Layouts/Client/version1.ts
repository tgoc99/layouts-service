...
const layoutsUuid = 'Layouts'
fin.Service.connect({ uuid: layoutsUuid }).then((channel:any) => {
    window.layoutsChannel = layoutsChannel = channel;
    // Any unregistered action will simply return false
    channel.defaultAction(()=>false);
})
...
const layoutsAPI = {
    ...
    onWillSaveLayout: (layoutDecorator: (layoutApp: LayoutApp) => LayoutApp|false): void => {
        layoutsChannel.register('willSaveLayout', layoutDecorator);
    }, 
